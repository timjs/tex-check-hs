{-# LANGUAGE TupleSections, NamedFieldPuns, DeriveDataTypeable #-}
module Main where
...
verbatim s
  =  (At, )                 <$> stripPrefix "@" s
 <|> (StartStop "typing", ) <$> stripPrefix "\\startstoptyping" s
...
opening s
  =  (Brace, )                  <$> stripPrefix "{" s
 <|> (Bracket, )                <$> stripPrefix "[" s
 <|> (Paren, )                  <$> stripPrefix "(" s
 <|> (Delimiter, )              <$> (stripPrefix "\\left" s >>= delimiter)
 <|> (\(n,r) -> (StartStop n, r))   <$> (stripPrefix "\\start" s >>= \s -> return (span isLetter s))
 -- <|> \n,r -> (BeginEnd n, r) <$> stripPrefix "\\begin{" s *> span isLetter -- FIXME
...
closing s
 ...
 <|> (Bracket, )                <$> stripPrefix "]" s
 <|> (Paren, )                  <$> stripPrefix ")" s
 <|> (Delimiter, )              <$> (stripPrefix "\\right" s >>= delimiter)
 <|> (\(n,r) -> (StartStop n, r))   <$> (stripPrefix "\\stop" s >>= \s -> return (span isLetter s))
 -- <|> \n,r -> (BeginEnd n, r) <$> stripPrefix "\\begin{" s *> span isLetter -- FIXME
